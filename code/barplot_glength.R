...
g_length <- ggplot(g_times, aes(x = Year, y = decimal_time, fill = Year)) +
  geom_bar(stat = "identity", position = "identity", color = "white", linewidth = 1.5) +
...
ggsave("barplot_glength.pdf", g_length, path="course-paper-or-presentation-lukenoel20/manuscript", height=4.2, width=6)